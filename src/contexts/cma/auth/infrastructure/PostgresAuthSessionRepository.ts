...
export class PostgresAuthSessionRepository implements AuthSessionRepository {
    ...
    async save(session: AuthSession): Promise<void> {
        await this.connection.execute("INSERT INTO cma__auth_session (id, user_id, expires, session_token) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO UPDATE SET expires_at = $3", [
            session.id,
            session.userId,
            session.expires,
    ...
    async search(id: string): Promise<AuthSession | null> {
        const session = await this.connection.searchOne<AuthSession>("SELECT id, user_id as userId, expires, session_token as sessionToken FROM cma__auth_session WHERE id = $1", [id]);
    ...
    async searchByToken(token: string): Promise<AuthSession | null> {
        const session = await this.connection.searchOne<AuthSession>("SELECT id, user_id as userId, expires, session_token as sessionToken FROM cma__auth_session WHERE session_token = $1", [token]);
