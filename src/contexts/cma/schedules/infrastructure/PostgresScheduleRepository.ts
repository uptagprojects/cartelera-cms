...
export class PostgresScheduleRepository implements ScheduleRepository {
    ...
    async save(schedule: Schedule): Promise<void> {
        ...
        const query =
            "INSERT INTO cma__schedules (id, name, start_date, finish_date, status) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (id) DO UPDATE SET name = $2, start_date = $3, finish_date = $4, status = $5";

        const values = [primitives.id, primitives.name, primitives.startDate, primitives.finishDate, primitives.status];
    ...
    async search(id: ScheduleId): Promise<Schedule | null> {
        const res = await this.connection.searchOne<DatabaseSchedule>(
            "SELECT id, name, start_date, finish_date, status FROM cma__schedules WHERE id = $1 LIMIT 1",
            [id.value]
        );
