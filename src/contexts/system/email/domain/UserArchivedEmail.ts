...
import { UserArchivedEmailSentDomainEvent } from "./event/UserArchivedEmailSentDomainEvent";

export type UserArchivedEmailPrimitives = {
    id: string;
    userId: string;
    userName: string;
    from: string;
...
export class UserArchivedEmail extends Email {
    ...
    static send(id: string, userId: string, name: string, emailAddress: string): UserArchivedEmail {
        const from = new EmailAddress(UserArchivedEmail.from);
        const subject = UserArchivedEmail.generateSubject(name);
        const html = UserArchivedEmail.generateHTML(name);
        const text = UserArchivedEmail.generateText(name);

        const email = new UserArchivedEmail(
            new EmailId(id),
            from,
        ...
        email.record(
            new UserArchivedEmailSentDomainEvent(
                id,
                userId,
                name,
                from.value,
    ...
    static fromPrimitives(primitives: UserArchivedEmailPrimitives): UserArchivedEmail {
        return new UserArchivedEmail(
      ...
      <tbody>
        <tr style="width:100%">
          <td>
            ...
            >
              Tu usuario ha sido eliminado del PNFi. Si deseas volver a utilizar
              el servicio, por favor, ponte en contacto con nosotros.
            </p>
            <p
              style="color:rgb(0,0,0);font-size:14px;line-height:24px;margin:16px 0"
            >
...
Tu usuario ha sido eliminado del PNFi. Si deseas volver a utilizar el servicio,
...
contacte con soporte@pnfi.pro y nos pondremos en contacto contigo.`
    ...
    toPrimitives(): UserArchivedEmailPrimitives {
        return {
            id: this.id.value,
