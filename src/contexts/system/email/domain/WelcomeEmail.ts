...
export class WelcomeEmail extends Email {
    ...
    static send(id: string, userId: string, name: string, emailAddress: string): WelcomeEmail {
        ...
        email.record(
            new WelcomeEmailSentDomainEvent(id, userId, name, from.value, emailAddress, subject.value, body.value)
        );
    ...
    private static generateBody(userName: string): EmailBody {
        return new EmailBody(`Bienvenido a la cartelera del PNFi, ${userName}! Completa tu perfil en https://cartelerapnfi.uptag.net/profile`);
    }
