...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
    ...
    try {
        ...
        ).publish(id, body.name, body.location, body.startDate.toISOString(), body.endDate.toISOString());
    } catch (error) {
...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
    ...
    try {
        ...
        if (error instanceof EventDoesNotExist) {
            return new Response(JSON.stringify({ code: "event_not_found", message: error.message }), {
                status: 404,
                headers: {
                    "Content-Type": "application/json"
                }
