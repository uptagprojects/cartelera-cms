...
import { AnnouncementAction, AnnouncementActionTypes, AnnouncementState, IManageAnnouncement } from "./types";
...
export function useGetAnnouncements(): {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const data = await customFetch(`/api/manage/announcements?${query}`).then(res => res.json());
...
export async function updateAnnouncementTitle(_state: { id: string; title: string }) {
    const titleSchema = z.string().min(1, "This field has to be filled.").max(120, "This field is too long.");
...
export async function updateAnnouncementContent(_state: { id: string; content: string }) {
    const contentSchema = z.string().min(1, "This field has to be filled.").max(320, "This field is too long.");
